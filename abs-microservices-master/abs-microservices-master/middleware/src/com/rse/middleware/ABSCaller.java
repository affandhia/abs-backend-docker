...
//    }
    ...
    public ABSUnit run() {
        try {
            ...
            if (routeUrl != null && routeUrl.length() > 0) {
                ...
                if (absrequest != null) {
                    try{
                        data = (ABSValue) controllerObject.getClass().getMethod(methodName, ABSHttpRequest_i.class).invoke(controllerObject, absrequest);
                    }
                    catch (InvocationTargetException e){
                        data = new ABS.StdLib.List_Nil();
                    }
                }
    ...
    public void setResponse(ABSValue data) throws Exception {
...
//                System.out.println(dataModels);
                ...
                String responseContent = DataTransformer.convertAbsObjectToJSON(data);
                
                if (data instanceof ABS.StdLib.List_Nil){
                    this.response.setStatus(404);
                    this.response.getWriter().write(responseContent);
                }
                else if (responseContent.equals("null")){
                    this.response.setStatus(404);
                }
                else {
                    this.response.getWriter().write(responseContent);
...
//            this.response += "<span style='font-size:0.8em;'>ABS Server: " + new Date() + "</span><br />";
    ...
    private ABSHttpRequest_i createABSHttpRequest(String[] roles) throws Exception {
        ...
        if (roles.length > 0) {
            //if there is roles limitation, need token
            if (token != null) {
                try {
